...
const azureFunc: AzureFunction = async (context: Context): Promise<void> => {
    ...
    // Find certificates that are about to expire and start a renewal process for them
    const certRequestsTasks: Promise<void>[] = [];
    ...
    for await (const blob of containerClient.listBlobsFlat()) {
        if (blob.name.endsWith('.json')) {
            ...
            if (certDetails === null) {
                ...
                context.log.verbose(`Renewing certificate for "${commonName}"`);
                certRequestsTasks.push(
                    acme2keyvault.run(context.log, credential, certRequest),
                );
            } else {
    ...
    await Promise.all(certRequestsTasks);
};
